...
public:
    ...
    // Метод с лямбда-функцией для фильтрации по статусу док-та
    // Если в метод не передали параметр статуса док-та,
    // то по умолчанию выдаются док-ты со статусом ACTUAL
    ...
    vector<Document> FindTopDocuments(const string& raw_query, Key key) const {
    ...
    vector<Document> FindAllDocuments(const Query& query, Key& key) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                ...
                if (key(document_id, current_document.status, current_document.rating)) {
