...
public:
    ...
    explicit SearchServer(const string& stop_words_text)
        : SearchServer(SplitIntoWords(stop_words_text)) { // Вызов делегирующего конструктора из контейнера строк
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if ((document_id < 0) || DocumentExist(document_id)) {
...
private:
    ...
    bool DocumentExist(const int& document_id) {
        return documents_.count(document_id) > 0;
    }
    ...
    static bool IsValidWord(const string& word) {
        // Правильное слово не должно содержать специальных символов
